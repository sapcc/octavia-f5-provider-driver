...
class HierachicalPortBindingDriver(aap.AllowedAddressPairsDriver):
    ...
    def get_segmentation_id(self, network_id, host):
        ...
        try:
            ...
            physical_network = self.get_physical_network(host)
    ...
    def get_physical_network(self, host):
        ...
        if host is None:
            base.NetworkException('No host given and no global physical network, failed retrieving physical network')

        try:
            f5_agents = self.neutron_client.list_agents(agent_type=constants.F5_NETWORK_AGENT_TYPE, host=host)
            device_mappings = f5_agents['agents'][0]['configurations']['device_mappings']
            for physical_network in device_mappings.keys():
                return physical_network
        except Exception as e:
            LOG.error('Error retrieving physical network dict for f5 network agent "%s": %s', host, e)
            raise e
