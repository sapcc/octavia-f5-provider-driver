...
def filter_cipher_suites(cipher_suites, object_name, object_id, http2=False):
    ...
    :param object_name: A printable representation of the object to be logged, e.g. "Listener" or "Pool".
    :param object_id: ID of the object the cipher suites belong to. This is used for logging, so it should be a string.
    :param http2: HTTP2 protocol using
...
def get_cipher_rule_and_group(ciphers, parent_obj, parent_id, http2=False):
    ...
    rule_args = {
        'cipherSuites': filter_cipher_suites(ciphers, parent_obj, parent_id, http2)
    }
    group_args = {
