...
class L7PolicyFlows(object):
    ...
    def get_create_l7policy_flow(self):
        ...
        create_l7policy_flow.add(database_tasks.MarkL7PolicyPendingCreateInDB(
            requires=constants.L7POLICY))
        create_l7policy_flow.add(f5_database_tasks.ReloadLoadBalancers(
            requires=constants.LOADBALANCER,
            provides=constants.LOADBALANCERS))
        create_l7policy_flow.add(f5_driver_tasks.TenantUpdate(
    ...
    def get_delete_l7policy_flow(self):
        ...
        delete_l7policy_flow.add(f5_driver_tasks.L7PolicyDelete(
            requires=[constants.L7POLICY,
                      constants.LISTENERS,
                      constants.LOADBALANCER,
                      constants.BIGIP]))
    ...
    def get_update_l7policy_flow(self):
        ...
        update_l7policy_flow.add(database_tasks.MarkL7PolicyPendingUpdateInDB(
            requires=constants.L7POLICY))
        update_l7policy_flow.add(f5_driver_tasks.L7PolicyUpdate(
            requires=[constants.L7POLICY,
                      constants.LISTENERS,
                      constants.LOADBALANCER,
                      constants.BIGIP]))
