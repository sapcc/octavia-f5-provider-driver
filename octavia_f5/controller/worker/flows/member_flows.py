...
class MemberFlows(object):
    ...
    def get_delete_member_flow(self):
        ...
        delete_member_flow.add(database_tasks.DeleteMemberInDB(
            requires=constants.MEMBER))
        delete_member_flow.add(f5_driver_tasks.ListenersUpdate(
            requires=[constants.LOADBALANCER,
                      constants.LISTENERS,
                      constants.BIGIP]))
    ...
    def get_update_member_flow(self):
        ...
        update_member_flow.add(database_tasks.MarkMemberPendingUpdateInDB(
            requires=constants.MEMBER))
        update_member_flow.add(f5_driver_tasks.ListenersUpdate(
            requires=[constants.LOADBALANCER,
                      constants.LISTENERS,
                      constants.BIGIP]))
    ...
    def get_batch_update_members_flow(self, old_members, new_members,
        ...
        # Update the Listener (this makes the changes active)
        batch_update_members_flow.add(f5_driver_tasks.ListenersUpdate(
            requires=(constants.LOADBALANCER,
                      constants.LISTENERS,
                      constants.BIGIP)))
