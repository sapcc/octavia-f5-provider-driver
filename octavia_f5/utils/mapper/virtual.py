...
class VirtualMapper(partition.PartitionMapper):
    def get_virtual_path(self, listener):
        name = constants.PREFIX_LISTENER + listener.id
        partition = self.get_partition_name(
            listener.project_id)
    ...
    def get_virtual(self, listener):
        ...
        pool = self.get_vip_default_pool(listener)
        ...
        vip = self._map_virtual(listener, pool=pool,
                           policies=listener_policies)

        return vip

    def _map_virtual(self, listener, pool=None, policies=None):
        loadbalancer = listener.load_balancer
        virtual = self.get_virtual_path(listener)

        virtual["description"] = self.get_resource_description(pool)

        if pool:
            pool_name = get_pool_path(loadbalancer, pool)
